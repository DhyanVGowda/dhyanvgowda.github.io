...
window.addEventListener("scroll", () => {
  ...
  console.log(
    "%c Designed and Developed by Dhyan Vasudeva Gowda ",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateGreeting() {
    ...
    if (greetingElement) {
      greetingElement.innerText = getTimeBasedGreeting();
    }
  }
  ...
  const emoji = document.getElementById("nav-avatar");
  if (emoji) {
    emoji.addEventListener("mouseover", showGreeting);
    emoji.addEventListener("mouseout", hideGreeting);
  }

  updateGreeting();
  cycleGreeting();

  setInterval(cycleGreeting, 2000);
